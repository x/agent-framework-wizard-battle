...
BASE_URL = os.getenv("WIZARD_URL", "http://localhost:8000")
...
def _start_server() -> subprocess.Popen | None:
    ...
    for _ in range(40):
        try:
            ...
            time.sleep(0.5)
    proc.kill()
    raise RuntimeError("Server failed to start within 20 s")


@pytest.fixture(scope="session", autouse=True)
def _server() -> None:  # type: ignore[return]
    proc = _start_server()
    yield
    if proc:
        proc.terminate()
...
def chat(sid: str | None, message: str) -> tuple[str, CharacterSheet | None, str]:
    """POST /chat, collect full SSE stream -> (text, sheet, session_id)."""
    text_parts: list[str] = []
...
def player_reply(history: list[dict], sheet: CharacterSheet) -> str:
    ...
    try:
        resp = completion(model=PLAYER_MODEL, messages=msgs, max_tokens=300)
        if resp.choices:
...
def test_wizard_creation() -> None:
    ...
    print(f"{B}  Server: {BASE_URL}  Player: {PLAYER_MODEL}  Judge: {JUDGE_MODEL}{X}")
    print(f"{B}{'═' * 72}{X}\n")
    ...
    # ── Sheet check ──────────────────────────────────────────────────────────
    errors = check_sheet(sheet)
    total = len(EXPECTED.model_dump(by_alias=True)) + 8  # expected fields + non-empty checks
